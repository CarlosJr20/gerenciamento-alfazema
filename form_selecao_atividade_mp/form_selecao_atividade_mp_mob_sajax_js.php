  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_selecao_atividade_mp_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_selecao_atividade_mp_mob_form();
	}
  ...
  // ---------- Validate nome_da_atividade
  function do_ajax_form_selecao_atividade_mp_mob_validate_nome_da_atividade()
  {
    var nomeCampo_nome_da_atividade = "nome_da_atividade";
    var var_nome_da_atividade = scAjaxGetFieldText(nomeCampo_nome_da_atividade);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_selecao_atividade_mp_mob_validate_nome_da_atividade(var_nome_da_atividade, var_script_case_init, do_ajax_form_selecao_atividade_mp_mob_validate_nome_da_atividade_cb);
  } // do_ajax_form_selecao_atividade_mp_mob_validate_nome_da_atividade

  function do_ajax_form_selecao_atividade_mp_mob_validate_nome_da_atividade_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "nome_da_atividade";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_selecao_atividade_mp_mob_validate_nome_da_atividade_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_form_selecao_atividade_mp_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_form_selecao_atividade_mp_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_nome_da_atividade = scAjaxGetFieldText("nome_da_atividade");
    var var_id_atividade = scAjaxGetFieldText("id_atividade");
    ...
    scAjaxProcOn();
    x_ajax_form_selecao_atividade_mp_mob_submit_form(var_nome_da_atividade, var_id_atividade, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_selecao_atividade_mp_mob_submit_form_cb);
  } // do_ajax_form_selecao_atividade_mp_mob_submit_form

  function do_ajax_form_selecao_atividade_mp_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("nome_da_atividade");
      scLigEditLookupCall();
<?php
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_form_selecao_atividade_mp_mob_submit_form_cb_after_alert);
  } // do_ajax_form_selecao_atividade_mp_mob_submit_form_cb
  function do_ajax_form_selecao_atividade_mp_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_selecao_atividade_mp_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_selecao_atividade_mp_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_selecao_atividade_mp_mob_navigate_form, function() {});
    } else {
      perform_ajax_form_selecao_atividade_mp_mob_navigate_form();
    }
  }

  function perform_ajax_form_selecao_atividade_mp_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxHideErrorDisplay("table");
    scAjaxHideErrorDisplay("nome_da_atividade");
    var var_id_atividade = document.F2.id_atividade.value;
    ...
    scAjaxProcOn();
    x_ajax_form_selecao_atividade_mp_mob_navigate_form(var_id_atividade, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search,  var_nmgp_cond_fast_search,  var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_selecao_atividade_mp_mob_navigate_form_cb);
  } // do_ajax_form_selecao_atividade_mp_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_selecao_atividade_mp_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_selecao_atividade_mp_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.id_atividade.value = scAjaxGetKeyValue("id_atividade");
    scAjaxSetSummary();
    scAjaxShowDebug();
    scAjaxSetLabel(true);
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_selecao_atividade_mp_mob_navigate_form_cb_after_alert);
  } // do_ajax_form_selecao_atividade_mp_mob_navigate_form_cb
  function do_ajax_form_selecao_atividade_mp_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_form_selecao_atividade_mp_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
    ...
    SC_btn_grp_text();
  } // do_ajax_form_selecao_atividade_mp_mob_navigate_form_cb_after_alert
  function sc_hide_form_selecao_atividade_mp_mob_form()
  {
  ...
  ajax_field_list[0] = "nome_da_atividade";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "nome_da_atividade": {"label": "Nome Da Atividade", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  var ajax_field_mult = {
    "nome_da_atividade": new Array()
  };
  ajax_field_mult["nome_da_atividade"][1] = "nome_da_atividade";

  var ajax_field_id = {
    "nome_da_atividade": new Array("hidden_field_label_nome_da_atividade", "hidden_field_data_nome_da_atividade")
  };

  var ajax_read_only = {
    "nome_da_atividade": "off"
  };
