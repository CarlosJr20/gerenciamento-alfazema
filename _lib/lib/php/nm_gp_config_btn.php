    ...
    $addAttr = '')
{
    ...
    if ('' != $sGrupo && '__sc_grp__' !=$sGrupo)
    {
        ...
        //if(!empty($sClick))
        {
            $sClick = "if (typeof(getAppData) == 'undefined') { scBtnGrpHide($(this).closest( '.SC_SubMenuApp' ).attr('id').substring(12), true)} " . $sClick;
        }
    }
    ...
    if ('button' == $arr_buttons[$sBtn]['type'])
    {
        ...
        {
            if ((isset($arr_buttons[$sBtn]['has_fa']) && $arr_buttons[$sBtn]['has_fa']) || (isset($arr_buttons[$sBtn]['display']) && strpos($arr_buttons[$sBtn]['display'], "fontawesomeicon") !== false))
			{
