...
//
    ...
    switch($_GET['step'])
    {
        ...
        case 6:
            echo "<br>** Convertendo Logradouros **";flush();
            conv_logradouros_unidade_op();
        break;
        case 7:
            echo "<br>** Criando arquivos de controle **";flush();
            arq_controles();
...
function conv_logradouros()
{
    ...
    echo "** Select das localidades **";flush();
    ...
    echo "** fim Select das localidades **";flush();
...
//--- Logradouros
function conv_logradouros_unidade_op()
{
    global $root, $Db;
    ...
    $comando = "SELECT LOG_LOGRADOURO.UFE_SG, LOG_UNID_OPER.CEP, LOG_LOGRADOURO.LOG_NO, LOG_LOGRADOURO.LOG_TIPO_LOGRADOURO, LOG_LOGRADOURO.LOG_COMPLEMENTO, LOG_BAIRRO.BAI_NO, LOG_LOCALIDADE.LOC_NO
      FROM LOG_UNID_OPER, LOG_LOGRADOURO, LOG_BAIRRO, LOG_LOCALIDADE
      WHERE
         LOG_UNID_OPER.LOG_NU_SEQUENCIAL = LOG_LOGRADOURO.LOG_NU_SEQUENCIAL AND
         LOG_LOGRADOURO.LOC_NU_SEQUENCIAL = LOG_LOCALIDADE.LOC_NU_SEQUENCIAL AND
         LOG_LOGRADOURO.BAI_NU_SEQUENCIAL_INI = LOG_BAIRRO.BAI_NU_SEQUENCIAL
      ORDER BY
         LOG_LOGRADOURO.UFE_SG, LOG_LOCALIDADE.LOC_NO, LOG_LOGRADOURO.LOG_NO";
    ...
    while (!$rs_logr->EOF)
    {
        ...
        if ($estado != $dados[0])
        {
            ...
            $estado = $dados[0];
            $cidade_nome = $dados[4];
            $arq_saida = fopen ($root . "/cep_logradouros_unidades_operacionais_" . strtolower($estado) . ".txt", 'w');
            //$arq_point = fopen ($root . "/cep_pointer_log_" . strtolower($estado) . ".txt", 'w');
            echo "<br>   >> Convertendo Estado: " . $estado;flush();
        }
        fwrite($arq_saida, $dados_saida) ;
