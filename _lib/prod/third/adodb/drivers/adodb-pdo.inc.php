...
class ADODB_pdo extends ADOConnection {
    ...
    function _connect($argDSN, $argUsername="", $argPassword="", $argDatabasename="", $arrExtraArgs=array(), $charset='')
    {
        ...
        if ('sqlite' != $this->dsnType && 'mysql' != $this->dsnType && 'pgsql' != $this->dsnType && 'sqlsrv' != $this->dsnType && 'utf8' == $charset) {
            $argDSN .= ';charset=utf8';
        }
        ...
        try {
            ...
            switch($this->dsnType)
            {
                ...
                case 'pgsql':
                    ...
                    break;
                case 'sqlsrv':
                    $this->_connectionID->exec("SET NOCOUNT ON");
                    if ('utf8' == $charset) {
                        $this->_connectionID->setAttribute(PDO::SQLSRV_ATTR_ENCODING, PDO::SQLSRV_ENCODING_UTF8);
                    }
                    else {
    ...
    {
        if(method_exists($this->_driver, "BlobEncode"))
        {
            return $this->_driver->BlobEncode($blob);
        }
        else
        {
            return $blob;
    ...
    {
        if(method_exists($this->_driver, "BlobDecode"))
        {
    ...
    {
        if(method_exists($this->_driver, "UpdateBlob"))
        {
            if ($this->debug) {
                ADOConnection::outp( "(pdo-". $this->_driver->dsnType ."): Specific Drive UpdateBlob($table,$column,val,$where,$blobtype)");
            }
        ...
        {
            if ($this->debug) {
                ADOConnection::outp( "UPDATE ". $table ." SET ". $column ." = :field_blob WHERE " . $where);
            }

            $this->prepare("UPDATE ". $table ." SET ". $column ." = :field_blob WHERE " . $where);

            if ('sqlsrv' == $this->dsnType) {
                $this->_stmt->bindParam(":field_blob", $val, PDO::PARAM_LOB, 0, PDO::SQLSRV_ENCODING_BINARY);
            } else {
