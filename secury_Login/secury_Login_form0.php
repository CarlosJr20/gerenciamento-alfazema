...
.sc-button-image.disabled img {
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>secury_Login/secury_Login_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
function nav_desliga_img()
{
 ...
 function nm_field_disabled(Fields, Opt) {
  ...
  for (i=0; i < Field.length; i++)
  {
     ...
     if (F_name == "links")
     {
        $('input[name="links"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="links"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="links"]').removeClass("scFormInputDisabled");
        }
     }
  }
 } // nm_field_disabled
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['recarga'];
}
    $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['dashboard_info']['remove_margin'] ? 'margin: 0; ' : '';
    $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['dashboard_info']['remove_border'] ? 'border-width: 0; ' : '';
...
<?php
$_SESSION['scriptcase']['error_span_title']['secury_Login'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['secury_Login'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
  if (!$this->Embutida_call && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['mostra_cab']) || $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['mostra_cab'] != "N") && (!$_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['dashboard_info']['under_dashboard'] || !$_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['dashboard_info']['compact_mode'] || $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['dashboard_info']['maximized']))
  {
?>
<tr><td>
<style>
...
<?php
  ...
  {
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['where_filter']))
       {
...
<span id="id_read_on_login" class="sc-ui-readonly-login css_login_line" style="<?php echo $sStyleReadLab_login; ?>"><?php echo $this->form_format_readonly("login", $this->form_encode_input($this->login)); ?></span><span id="id_read_off_login" class="css_read_off_login<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_login; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=35"; } ?> maxlength=20 alt="{datatype: 'text', maxLength: 20, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: false, enterSubmit: true, autoTab: false, selectOnFocus: false, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
 </TD>
   <?php }?>
...
<span id="id_read_on_pswd" class="sc-ui-readonly-pswd css_pswd_line" style="<?php echo $sStyleReadLab_pswd; ?>"><?php echo $this->form_format_readonly("pswd", $this->form_encode_input($this->pswd)); ?></span><span id="id_read_off_pswd" class="css_read_off_pswd<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_pswd; ?>"><div class="scFormObjectOddPwdBox"><input class="sc-js-input scFormObjectOddPwdInput scFormObjectOddPwdText sc-ui-pwd-toggle css_pswd_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_pswd" type="password" autocomplete="off" name="pswd" value="" 
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=31"; } ?> maxlength=20 alt="{datatype: 'text', maxLength: 20, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: false, enterSubmit: true, autoTab: false, selectOnFocus: false, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ><span id="id_pwd_show_hide_pswd" class="sc-ui-pwd-toggle-icon"><i class="fa fa-eye sc-ui-pwd-eye" aria-hidden="true" id="id_pwd_fa_pswd"></i></span></div></span><?php } ?>
 </TD>
   <?php }?>
...
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['Lookup_remember_me'][] = '1'; ?>
...
?>
       <?php echo nmButtonOutput($this->arr_buttons, "bok", "scBtnFn_sys_format_ok()", "scBtnFn_sys_format_ok()", "sub_form_b", "", " Login ", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "sc-unique-btn-1", "", "");?>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['masterValue']))
{
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("secury_Login");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("secury_Login");
 parent.scAjaxDetailHeight("secury_Login", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['secury_Login']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
