...
$old_error_handler = set_error_handler("form_cliente_mp_scriptcase_error_handler");

function form_cliente_mp_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_cliente_mp']['contr_erro']) && $_SESSION['scriptcase']['form_cliente_mp']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_cliente_mp->contr_form_cliente_mp->Apl_com_erro = true;
            if ($inicial_form_cliente_mp->contr_form_cliente_mp->NM_ajax_flag)
            {
                $inicial_form_cliente_mp->contr_form_cliente_mp->NM_ajax_info['errList']['geral_form_cliente_mp'][] = $msg;
                return;
            }
            form_cliente_mp_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_cliente_mp_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_cliente_mp'])  && '' != $_SESSION['scriptcase']['error_icon']['form_cliente_mp'])  ? $_SESSION['scriptcase']['error_icon']['form_cliente_mp']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_cliente_mp']) && '' != $_SESSION['scriptcase']['error_close']['form_cliente_mp']) ? $_SESSION['scriptcase']['error_close']['form_cliente_mp'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_cliente_mp']) && '' != $_SESSION['scriptcase']['error_span_title']['form_cliente_mp']) ? $_SESSION['scriptcase']['error_span_title']['form_cliente_mp'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_cliente_mp']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_cliente_mp']) ? $_SESSION['scriptcase']['error_icon_title']['form_cliente_mp'] : '';
?>
