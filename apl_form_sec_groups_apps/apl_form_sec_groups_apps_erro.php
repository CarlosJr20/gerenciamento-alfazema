...
$old_error_handler = set_error_handler("apl_form_sec_groups_apps_scriptcase_error_handler");

function apl_form_sec_groups_apps_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['apl_form_sec_groups_apps']['contr_erro']) && $_SESSION['scriptcase']['apl_form_sec_groups_apps']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_apl_form_sec_groups_apps->contr_apl_form_sec_groups_apps->Apl_com_erro = true;
            if ($inicial_apl_form_sec_groups_apps->contr_apl_form_sec_groups_apps->NM_ajax_flag)
            {
                $inicial_apl_form_sec_groups_apps->contr_apl_form_sec_groups_apps->NM_ajax_info['errList']['geral_apl_form_sec_groups_apps'][] = $msg;
                return;
            }
            apl_form_sec_groups_apps_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function apl_form_sec_groups_apps_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['apl_form_sec_groups_apps'])  && '' != $_SESSION['scriptcase']['error_icon']['apl_form_sec_groups_apps'])  ? $_SESSION['scriptcase']['error_icon']['apl_form_sec_groups_apps']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['apl_form_sec_groups_apps']) && '' != $_SESSION['scriptcase']['error_close']['apl_form_sec_groups_apps']) ? $_SESSION['scriptcase']['error_close']['apl_form_sec_groups_apps'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['apl_form_sec_groups_apps']) && '' != $_SESSION['scriptcase']['error_span_title']['apl_form_sec_groups_apps']) ? $_SESSION['scriptcase']['error_span_title']['apl_form_sec_groups_apps'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['apl_form_sec_groups_apps']) && '' != $_SESSION['scriptcase']['error_icon_title']['apl_form_sec_groups_apps']) ? $_SESSION['scriptcase']['error_icon_title']['apl_form_sec_groups_apps'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['apl_form_sec_groups_apps']['error_buffer'] .= ob_get_flush();
}
