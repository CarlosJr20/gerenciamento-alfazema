...
$old_error_handler = set_error_handler("form_novidade_imog_scriptcase_error_handler");

function form_novidade_imog_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_novidade_imog']['contr_erro']) && $_SESSION['scriptcase']['form_novidade_imog']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_novidade_imog->contr_form_novidade_imog->Apl_com_erro = true;
            if ($inicial_form_novidade_imog->contr_form_novidade_imog->NM_ajax_flag)
            {
                $inicial_form_novidade_imog->contr_form_novidade_imog->NM_ajax_info['errList']['geral_form_novidade_imog'][] = $msg;
                return;
            }
            form_novidade_imog_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_novidade_imog_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_novidade_imog'])  && '' != $_SESSION['scriptcase']['error_icon']['form_novidade_imog'])  ? $_SESSION['scriptcase']['error_icon']['form_novidade_imog']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_novidade_imog']) && '' != $_SESSION['scriptcase']['error_close']['form_novidade_imog']) ? $_SESSION['scriptcase']['error_close']['form_novidade_imog'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_novidade_imog']) && '' != $_SESSION['scriptcase']['error_span_title']['form_novidade_imog']) ? $_SESSION['scriptcase']['error_span_title']['form_novidade_imog'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_novidade_imog']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_novidade_imog']) ? $_SESSION['scriptcase']['error_icon_title']['form_novidade_imog'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_novidade_imog']['error_buffer'] .= ob_get_flush();
}
